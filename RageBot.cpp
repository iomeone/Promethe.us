	...
	if (TargetID >= 0 && pTarget)
	{
		...
		if (g_Options.Ragebot.AutoStop)
		{
			...
			case 1:
				...
				break;
			case 2:
				ragebot::FakeWalk(pCmd, bSendPacket, pLocal);
			};
		}
...
//			HitBoxesToScan.push_back(RightShin);
	...
	static vector<int> baim{ UpperChest ,Chest ,Stomach ,Pelvis ,LeftUpperArm ,RightUpperArm ,LeftThigh,RightThigh ,LeftHand ,RightHand,LeftLowerArm,RightLowerArm };
...
void ragebot::FakeWalk(CInput::CUserCmd* cmd, bool &bSendPacket, C_BaseEntity *local)
{

	//	C_BaseEntity *local = g_EntityList->GetClientEntityFromHandle(g_Engine->GetLocalPlayer());

	static int choking = -1;
	choking++;
